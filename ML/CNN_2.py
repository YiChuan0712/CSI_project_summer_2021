...
X_train0 = data01.iloc[:, 5:905]
Y_train = data01.iloc[:, data0.columns == "bin_type"]

X_test0 = data02.iloc[:, 5:905]
...
X_train0 = X_train0.reshape((X_train0.shape[0], 30, 30, 1)).astype('float32')
X_test0 = X_test0.reshape((X_test0.shape[0], 30, 30, 1)).astype('float32')
...
X_train1 = data11.iloc[:, 5:905]

X_test1 = data12.iloc[:, 5:905]
...
X_train1 = X_train1.reshape((X_train1.shape[0], 30, 30, 1)).astype('float32')
X_test1 = X_test1.reshape((X_test1.shape[0], 30, 30, 1)).astype('float32')

""""""
X_train = np.concatenate((X_train0, X_train1), axis=3)
X_test = np.concatenate((X_test0, X_test1), axis=3)
...
model.add(Conv2D(64, (5, 5), input_shape=(30, 30, 2), activation='relu', data_format="channels_last", padding="same"))
model.add(Conv2D(64, (5, 5), input_shape=(30, 30, 2), activation='relu', data_format="channels_last", padding="same"))
